...
* 新增 `Handlers.JsonModel`，用于注册 JSON 方块/物品模型。
* 支持从 `assets/<modid>/blockstates`、`models/block`、`models/item` 加载 JSON 模型，并接入方块世界、物品栏、掉落物和手持渲染。
...
* 修复了矿物生成的注册在下界崩溃的问题
    * `MinableWorldGen`新增了`NETHER_RANDOM_HEIGHT`以供下界矿石使用
...
* 修复了矿物生成的注册在下界无效的问题
    * 若需要自定义逻辑, 可改为在`BiomeDecoration`中注册
* `EntityEvent`补上了`ServerPlayer`死亡事件
...
* 添加了`Platform`供查阅平台信息(如是否为实验版mite)。感谢`洗玻璃呀`
* 添加了从`AE2`借鉴来的本地化键名枚举`LocalizationEnum`
* 添加了`BiomeSpawnUtil`, 用于添加生物在特定群系生成
* 添加了`EntityEventHandler`处理实体事件
...
* `BiomeGenerate`现在允许修改玩家可出生群系
* `Tooltip`事件现在支持更多位置的注入
* 现在`PacketByteBuf`能拿到内部的IO流了
...
* `Tooltip`事件现在提供`itemStack`信息了
* 现在地下世界群系生成不会有平原了(测试代码导致)
* 修复了特殊熔炉配方会无视热值执行的问题
