...
* 现在`CraftingRecipeRegisterEvent`开放游戏内置合成表和模组注册合成表的访问, 如果你需要删除或修改, 请用`registerPost`确保你的处理优先级最高
* 现在配置文件不用中文作为键名,修复了配置读取的问题
* 覆写了`Packet24MobSpawn`以支持更多实体类型(如SitMod)
* 现在一些覆写可以根据配置决定是否应用
* 添加了`ExperimentalReleaseUtil`, 用于增加对普通版的兼容(现在可兼容普通版。感谢`洗玻璃呀`)
* 删除了关于地下世界群系生成的测试代码
